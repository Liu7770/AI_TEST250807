...
test.describe('Base64 Encoder/Decoder 测试', () => {
  ...
  test('清空功能测试', async ({ page }) => {
    ...
    // 执行清空操作
    await TestHelpers.performClear({
      page
    });
    
    // 验证输入框已清空
    await TestHelpers.assertInputValue({
  ...
  test('长文本编码测试', async ({ page }) => {
    ...
    // 验证有输出结果（不是默认占位符）
    await TestHelpers.assertOutputNotDefault({
      outputLocator: page.locator('div.w-full.h-80.overflow-auto'),
  ...
  test('编码功能测试 - 输入空格字符', async ({ page }) => {
    ...
    if (isDisabled) {
      const errorMessage = TestHelpers.createTestFailureMessage(
        ...
        '编码按钮应该为可用状态（空格是有效字符）',
        '编码按钮被禁用',
        '网站应该将空格视为有效输入，允许用户进行编码操作。这是网站设计缺陷。'
      );
  ...
  test('编码功能测试 - 输入换行符', async ({ page }) => {
    ...
    if (isDisabled) {
      const errorMessage = TestHelpers.createTestFailureMessage(
        '换行符编码按钮状态测试',
        '输入包含换行符的文本',
        '编码按钮应该为可用状态（换行符是有效字符）',
        '编码按钮被禁用',
        '网站应该将换行符视为有效输入，允许用户进行编码操作。这是网站设计缺陷。'
      );
      throw new Error(errorMessage);
    }
